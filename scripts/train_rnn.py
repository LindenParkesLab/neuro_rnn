...
kernels = ['euclidean', 'additive', 'comet']
runs = 50

for kernel_type in kernels:
    hidden_weights = np.zeros((runs, hidden_size, hidden_size))
    ...
    for run in np.arange(runs):
        ...
        file_str = '{:}_{:}-{:}-{:}_{:}-{:}-{:}'.format(task,
                                                        hidden_size, n_epochs, learning_rate,
                                                        kernel_type, reg_type, reg_weight)
        if run == 0:
            # save model parameters
            torch.save(model.state_dict(), os.path.join(modeldir, file_str + '.pt'))
        hidden_weights[run, :, :] = model.rnn.all_weights[0][1].detach().cpu().numpy()

    # save prediction results
    np.save(os.path.join(modeldir, file_str), hidden_weights)
